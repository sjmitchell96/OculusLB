...
void prepareLattice( SuperLattice3D<T,DESCRIPTOR>& sLattice,
		    ...
		    // wallFunctionParam<T>& wfParam,
		     Dynamics<T,DESCRIPTOR> &wfBoundaryDynamics) {
  ...
  wfParam.curved = true;
  wfParam.bodyForce = true;
  wfParam.wallProfile = 0;
  wfParam.rhoMethod = 1;
  wfParam.fneqMethod = 1;
  wfParam.latticeWalldistance = 0.5;
  wfParam.vonKarman = 0.375;

  // Material=5 -->wall function
  sLattice.defineDynamics( superGeometry, 5, &wfBoundaryDynamics );
  bc.addWallFunctionBoundary(superGeometry,5,converter,wfParam);
...
int main( int argc, char* argv[] ) {
  ...
  //Object for wall function parameters
  //wallFunctionParam<T> wfParam;//{1,1,0,false,true,true,0.375,0.5};
  //wfParam.curved = false;
  //wfParam.bodyForce = false;
  //wfParam.wallProfile = 0;
  //wfParam.rhoMethod = 1;
  //wfParam.fneqMethod = 1;
  //wfParam.latticeWalldistance = 0.5;
  ...
  ExternalTauEffLESBGKdynamics<T,DESCRIPTOR> wfBoundaryDynamics( converter.getLatticeRelaxationFrequency(),
  	instances::getBulkMomenta<T,DESCRIPTOR>(),0.1);
...
//CREATE ANOTHER ON LATTICE BC FOR WF ONLY - SEE IF THIS WORKS BETTER!!
  prepareLattice( sLattice, converter, bulkDynamics, sBoundaryCondition, sOffBoundaryCondition,
  	stlReader, superGeometry/*, wfParam*/, wfBoundaryDynamics );
...
//try not passing wf object - instntiate in preparelattice
//look at olb1.4 to see which bc needed for wf - local or interp
//try a forced example w/ different descriptor and force = 0 if this fails
//use gdb to find failure point
//cx1 to find ram load
