...
void setupRefinement(Grid3D<T,DESCRIPTOR>& coarseGrid,
  ...
  const Vector<T,3> hn4 = {0.05*chord,0.18*chord,0.05*chord}; //x,y,z heights in negative direction //Innermost
  const Vector<T,3> hp4 = {0.05*chord,0.18*chord,0.05*chord}; // '' positive

  const Vector<T,3> hn3 = {0.15*chord,0.28*chord,0.15*chord};
  const Vector<T,3> hp3 = {0.15*chord,0.28*chord,0.15*chord};

  const Vector<T,3> hn2 = {0.35*chord,0.48*chord,0.35*chord};
  const Vector<T,3> hp2 = {1.0*chord,0.48*chord,0.35*chord};

  const Vector<T,3> hn1 = {0.75*chord,0.88*chord,0.75*chord}; //Outermost
  const Vector<T,3> hp1 = {2.0*chord,0.88*chord,0.75*chord};
...
void prepareLattice(Grid3D<T,DESCRIPTOR>& grid, STLreader<T>& stlReader, const bool& bouzidiOn,
	const bool& wallFunctionOn, const bool& startUpOn, const wallFunctionParam<T>& wfParam) {
	...
	//Wall function boundary dynamics
	Dynamics<T,DESCRIPTOR>& wfBoundaryDynamics = grid.addDynamics(
			std::unique_ptr<Dynamics<T,DESCRIPTOR>>(
				new ExternalTauEffLESBGKdynamics<T,DESCRIPTOR>(
					omega, instances::getBulkMomenta<T,DESCRIPTOR>(),0.1)));
...
//	sLattice.defineDynamics(sGeometry, 2, &bulkDynamics);
	...
	else if ( wallFunctionOn ) {
		std::cout << "start wf" << std::endl;
		sLattice.defineDynamics(sGeometry, 5, &wfBoundaryDynamics);
		std::cout << "interim" << std::endl;
		onbc.addWallFunctionBoundary(sGeometry, 5, converter, wfParam);
		std::cout << "end wf" << std::endl;
	}
	else {
...
void getBladeSectionPressure(Grid3D<T,DESCRIPTOR>& grid, int iT, int& nAv,
				...
				bool timeAvg, std::vector<T> bladeMin, T chord, T thickness,
	...
	T z {sectionSpan*span};
	  ...
	  std::vector<T> surface_x{0.0, 0.0000805780000000027, 0.000283419999999999,
		...
		for(std::vector<T>::size_type j = 0; j < surface_x.size(); ++j) {
			const T point[3] {bladeMin[0] + surface_x[j], bladeMin[1] + surface_y[j],
				 		bladeMin[2] + z};
...
int main( int argc, char* argv[] ) {
	...
	//Blade parameters
	const T chord = 0.04523;
	const T thickness = 0.003137;
        const T span = 0.040;
	const T theta = 0.0; //Pitch angle (+ve = anticlockwise)
	const std::vector<T> bladeMin = {4.*chord,3.*chord-0.5*thickness,3.5*chord-0.5*span}; //Origin of blade bounding box

	//Domain and simulation parameters
	const int N = 10;        // resolution of the model (coarse cells per chord)
	const int nRefinement = 1;	//Number of refinement levels (current max = 4)
	const T lDomainPhysx = 12.*chord; //Length of domain in physical units (m)
	const T lDomainPhysy = 6.*chord;
	const T lDomainPhysz = 7.*chord;
	const T maxPhysT = 10000.; // max. simulation time in s, SI unit
	...
	const bool wallFunctionOn = false;//if bouzidi = false, true = wall function

	//File name definitions
	const std::string wingStlFile = "lyonWing_domain2_0deg_standard.stl";	//Stl mesh for wing
	...
	//Object for wall function boundary
	wallFunctionParam<T> wfParam{1,1,0,false,true,true,0.375,0.5};
	...
	STLreader<T> stlReader( wingStlFile, coarseGrid.getConverter().getPhysDeltaX()*std::pow(0.5,nRefinement), 0.001, 1, true );
	...
	coarseGrid.forEachGrid(prepareLattice,stlReader,bouzidiOn,wallFunctionOn,startUpOn,wfParam);
	...
	util::ValueTracer<T> rhoTracer(/*coarseGrid.getConverter().getLatticeTime(0.01)*/1000, 1e-5);
  	...
  	for ( int iT = 0; iT < coarseGrid.getConverter().getLatticeTime( maxPhysT ); ++iT ) {
		...
		if ( (iT > iTAvg) && (iT % pressureIter == 0) ) {
			...
			wingSection1CpTimeAvgFile,wingSec1TimeAvg,bladeMin,
