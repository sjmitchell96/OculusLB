...
void prepareGeometry( Grid3D<T,DESCRIPTOR>& grid,
  ...
  //cout << "ORIGIN CHECK" << endl;
  //cout << "FINE GRID ORIGIN" << grid.getOrigin()[0] << endl;
  cout << "FINE GRID DOMAIN ORIGIN" << grid.getSuperGeometry().getStatistics().getMinPhysR(0)[0] << endl;
...
//OLD WAY
  sGeometry.rename(0,2,extendedDomain2);
  sGeometry.rename(2,1,stlReader);

  const Vector<T,3> bladeBoxOrigin {bladeCr[0] - 0.04523, bladeCr[1] - 0.04523,origin[2] - deltaX };
  const Vector<T,3> bladeBoxExtend {2*0.04523, 2*0.04523, origin[2]+extend[2]+2*deltaX};
...
void setupRefinement(Grid3D<T,DESCRIPTOR>& coarseGrid,
	...
	//Heights around wing box for each refinement level
  	const Vector<T,2> hn4 = {0.25*chord,0.3*chord}; //x,y heights in negative direction //Innermost
	...
	if(n >= 1) {
		...
		Vector<T,3> fineOrigin = {bladeOrigin[0]-hn1[0],bladeOrigin[1]-hn1[1],domainOrigin[2]};
			...
			if(n>=3) {
				...
				if(n>=4) {
					...
					Vector<T,3> fineOrigin4 = {bladeOrigin[0]-hn4[0],bladeOrigin[1]-hn4[1],
	 				...
	 				Vector<T,3> fineExtend4 = {chord+hp4[0]+hn4[0],thickness+hp4[1]+hn4[1],
...
int main( int argc, char* argv[] ) {
	...
	const std::string wingStlFile = "lyonBlade_q2d_0deg_standard.stl";	//Stl mesh for wing
	...
	const int pressureIter = 1000; //time averaged pressure capture
	const int checkIter 	 = 1000;
	const int bladeForceIter = 10;
	const int iTAvg = 10000; //Timesteps after which to start time-averaging results
	...
	Grid3D<T,DESCRIPTOR>& wingGrid = coarseGrid.locate(Vector<T,3>(bladeCr[0],bladeCr[1],bladeCr[2]));
	...
	std::vector<std::unique_ptr<SuperLatticePhysWallShearStress3D<T,DESCRIPTOR>>> wss;
	//std::vector<std::unique_ptr<SuperLatticeTimeAveragedF3D<T>>> sAveragedP;
	...
	coarseGrid.forEachGrid([&](Grid3D<T,DESCRIPTOR>& grid,
		...
		int& i_grid) {
			...
			wss.push_back(std::unique_ptr<SuperLatticePhysWallShearStress3D<T,DESCRIPTOR>>(
			new SuperLatticePhysWallShearStress3D<T,DESCRIPTOR>(sLattice,sGeometry,2,converter,stlReader)));
			yPlus.push_back(std::unique_ptr<SuperLatticeYplus3D<T,DESCRIPTOR>>(
			new SuperLatticeYplus3D<T,DESCRIPTOR>(sLattice,converter,sGeometry,stlReader,5)));
			i_grid++;
		},sVel, sP, sAveragedVel, sAveragedP, wss, yPlus, stlReader, i_grid);
		...
		//Add time-averaged functor vector into getVTK functions
			...
			i_grid = 0;	
      			coarseGrid.forEachGrid("aerofoil3D",
						...
						std::vector<std::unique_ptr<SuperLatticePhysWallShearStress3D<T,DESCRIPTOR>>>& wssVector,
						...
						int& i_grid){
							...
							SuperLatticePhysWallShearStress3D<T,DESCRIPTOR>& wss= *wssVector[i_grid];
