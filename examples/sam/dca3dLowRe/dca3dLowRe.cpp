...
//#define DESCRIPTOR WALEForcedD3Q19Descriptor
...
void setupRefinement(Grid3D<T,DESCRIPTOR>& coarseGrid,
  ...
  const Vector<T,2> hn4 = {0.05 * chord, 0.2 * chord}; 
  const Vector<T,2> hp4 = {0.05 * chord, 0.2 * chord}; // '' positive

  const Vector<T,2> hn3 = {0.25 * chord, 0.4 * chord};
  const Vector<T,2> hp3 = {0.35 * chord, 0.4 * chord};

  const Vector<T,2> hn2 = {0.45 * chord, 0.6 * chord};
  const Vector<T,2> hp2 = {0.95 * chord, 0.6 * chord};

  const Vector<T,2> hn1 = {0.65 * chord, 0.8 * chord}; //Outermost
  const Vector<T,2> hp1 = {1.75 * chord, 0.8 * chord};

  if(n >= 1) {
    // Refinement around the wing box - level 1
    T coarseDeltaX = coarseGrid.getConverter().getPhysDeltaX();
    Vector<T,3> fineOrigin = {bladeBoxOrigin[0] - hn1[0],
	                      bladeBoxOrigin[1] - hn1[1],
			      domainOrigin[2]};
    Vector<T,3> fineExtend = {chord+hp1[0] + hn1[0],
	                      thickness + hp1[1] + hn1[1], 
			      domainExtend[2]};
    ...
    Vector<T,3> innerOrigin   = origin + Vector<T,3> {coarseDeltaX,
	                                              coarseDeltaX,
						      0.};
    ...
    coarseGrid.addCoarseCoupling(fineGrid, 
    ...
    if(n >= 2) {
      ...
      origin = fineGrid2.getOrigin() + Vector<T,3> {0.,
	                                            0.,
						    0.5 * coarseDeltaX};
      extend = fineGrid2.getExtend() - Vector<T,3> {0.,
	                                            0.,
						    0.5 * coarseDeltaX};
      ...
      fineGrid.addCoarseCoupling(fineGrid2,
		                 innerOrigin + innerExtendX,
				 innerExtendYZ);
      fineGrid.addCoarseCoupling(fineGrid2,
		                 innerOrigin + innerExtendY,
	...
	if(n >= 4) {
	  ...
	  Vector<T,3> fineExtend4 = {chord + hp4[0] + hn4[0],
		                    thickness + hp4[1] + hn4[1],
				    domainExtend[2] + deltaX0 + deltaX1
			            + deltaX2};

	  auto& fineGrid4 = fineGrid3.refine(fineOrigin4, fineExtend4,
				             false, false, true, false);
	  prepareGeometry(fineGrid4, 
			  domainOrigin, 
			  domainExtend, 
			  indicatorBlade);

	  origin = fineGrid4.getOrigin() + Vector<T,3>{0., 
		                                       0.,
						       0.5 * coarseDeltaX};
	  extend = fineGrid4.getExtend() - Vector<T,3>{0.,
		                                       0.,
						       0.5 * coarseDeltaX};
	  extendXZ = {extend[0], 0., extend[2]};
	  extendYZ = {0., extend[1], extend[2]};
...
void prepareLattice(Grid3D<T,DESCRIPTOR>& grid,
	...
	Dynamics<T,DESCRIPTOR>& bulkDynamics = grid.addDynamics(
			std::unique_ptr<Dynamics<T,DESCRIPTOR>>(
				...
				//new WALEForcedBGKdynamics<T,DESCRIPTOR>(
			        //omega, instances::getBulkMomenta<T,DESCRIPTOR>(),0.1)));	
...
void getBladeForce(Grid3D<T,DESCRIPTOR>& grid,
	...
	SuperLatticePhysDragBlade3D<T,DESCRIPTOR> drag( sLattice, superGeometry, std::vector<int>{5,6}, converter,span*chord  );
	T bladeForce[3];
	int input1[0];
...
int main( int argc, char* argv[] ) {
	...
	const T span = 6. * chord;
	const T r1 = 0.1836;
	const T r2 = 0.00015;
	const T xp = 0.02538;
	const T theta = 0.00; //Pitch (+ve = anticlockwise)
	const Vector<T,3> bladeOrigin = {4.5 * chord,
	       	                         4. * chord,
					 -1. * chord}; //Origin of blade

	//Domain and simulation parameters
	const int N = 24;        // resolution of the model (coarse cells per chord)
	...
	// Indicator for blade
	IndicatorBladeDca3D<T> blade(bladeOrigin,chord, thickness, span, r1, r2, xp, theta);
	...
	coarseGrid.forEachGrid(prepareLattice,blade,bouzidiOn,startUpOn);
	clout << "Total number of active cells: " << coarseGrid.getActiveVoxelN() << std::endl;

	//Reference to finest grid containing wing
	Grid3D<T,DESCRIPTOR>& wingGrid = coarseGrid.locate(Vector<T,3>(bladeOrigin[0],bladeOrigin[1],bladeOrigin[2]+span/2.));
	...
	coarseGrid.forEachGrid([&](Grid3D<T,DESCRIPTOR>& grid,
		...
		int& i_grid) {
			...
			new SuperLatticePhysWallShearStressAndPressure3D<T,DESCRIPTOR>(sLattice,sGeometry,6,converter,indicatorBlade)));
      			...
      			coarseGrid.forEachGrid("aerofoil3Dplane1",
						...
						int& i_grid){
							...
							SuperLatticePhysWallShearStressAndPressure3D<T,DESCRIPTOR>& wssp = *wsspVector[i_grid];
                       					SuperLatticeTimeAveragedF3D<T>& sAveragedWSSP = *sAveragedWSSPVector[i_grid];
							SuperLatticeYplus3D<T,DESCRIPTOR>& yPlus = *yPlusVector[i_grid];
							getVTK2D(grid, id, iT, sAveragedVel,sAveragedP,wssp,sAveragedWSSP, yPlus);
							i_grid++;
							});
      						clout << "Get results vtk" << endl;
		}
